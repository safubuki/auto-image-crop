...
- **顔の向きを検出し、視線の先に空間を空けるようにクロップ位置を自動調整**
...
1.  **顔のサイズ評価 (40%)**：
    ...
    -   計算式（概念）： `size_score = (顔の面積) / (検出された最大の顔の面積)`
    -   大きい顔ほど主要な被写体である可能性が高いという考えに基づく

2.  **中央度評価 (40%)**：
    -   顔が画像の中心に近いほど高スコア
    -   計算式（概念）： `center_score = 1.0 - (顔の中心と画像中心の距離) / (画像中心から最も遠い角までの距離)`
    -   中央に位置する顔ほど主要な被写体である可能性が高いという考えに基づく

3.  **鮮明度評価 (20%)**：
    -   顔がより鮮明であるほど高スコア
    -   計算式（概念）： `sharpness_score = (顔領域の鮮明度) / (検出された最も鮮明な顔の鮮明度)`
...
- 顔領域を10%拡張して認識精度を向上
- 複数の顔が検出された場合は上記のスコアリングに基づいて最適な顔を選択
- 顔の中心位置と大きさを正確に特定
...
- **顔の矩形表示**：
  - クロップの基準となった主要な顔（スコア最高）：**緑色**の矩形（線幅2）で表示
...
- **クロップ領域の表示**:
  - **左側の元画像プレビュー**に、**赤色の太い矩形**で最終的なクロップ領域を表示。
  - どの部分が切り出されるかを事前に確認可能。

- **構図グリッドの表示**：
  - **右側のクロップ後プレビュー**に、緑色の線で選択中の構図ライン（三分割法またはファイグリッド）を表示。
...
5.  近い方のラインに顔の中心が来るようにクロップ位置を水平方向に調整。
    **さらに、顔が左右どちらかを向いている場合は、視線の先に空間ができるように調整します（例：右向きなら顔を左のラインに、左向きなら顔を右のラインに配置）。**
